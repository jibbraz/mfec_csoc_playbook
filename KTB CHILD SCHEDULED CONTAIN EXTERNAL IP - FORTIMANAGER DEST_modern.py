...
def get_the_top_ip_address_from_the_list(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None, custom_function=None, **kwargs):
    ...
    import ipaddress
    success, message, iplist = phantom.get_list(list_name='Fortimanager - Dest IP to contain')
    for ip in iplist:
        try:
...
def decision_1(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None, custom_function=None, **kwargs):
    ...
    if matched:
        prepare_dest_ip_artifact(action=action, success=success, container=container, results=results, handle=handle, custom_function=custom_function)
        return
...
def remove_the_top_ip_address_from_the_list(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None, custom_function=None, **kwargs):
    ...
    phantom.delete_from_list(list_name="Fortimanager - Dest IP to contain", value=get_the_top_ip_address_from_the_list__top_ip_address, column=None, remove_all=False, remove_row=False)
    success, message, iplist = phantom.get_list(list_name='Fortimanager - Dest IP to contain')
    for ip in iplist:
        try:
            ipaddress.ip_address(ip[0])
...
"""
Fortimanager Block Dest IP
"""
@phantom.playbook_block()
def fortimanager_block_dest_ip(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None, custom_function=None, **kwargs):
    phantom.debug('fortimanager_block_dest_ip() called')
    
    get_the_top_ip_address_from_the_list__top_ip_address = json.loads(phantom.get_run_data(key='get_the_top_ip_address_from_the_list:top_ip_address'))

    # call playbook "local/PLAYBOOK-CONTAIN-INDICATOR-CONSOLIDATED-FORTIMANAGER-DSTIP-01", returns the playbook_run_id
    playbook_run_id = phantom.playbook(playbook="local/PLAYBOOK-CONTAIN-INDICATOR-CONSOLIDATED-FORTIMANAGER-DSTIP-01", container=container, name=f"fortimanager_block_dest_ip_{get_the_top_ip_address_from_the_list__top_ip_address}", callback=sleep)

    return

"""
Add dest IP artifact
"""
@phantom.playbook_block()
def add_dest_ip_artifact(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None, custom_function=None, **kwargs):
    phantom.debug('add_dest_ip_artifact() called')
        
    #phantom.debug('Action: {0} {1}'.format(action['name'], ('SUCCEEDED' if success else 'FAILED')))
    
    get_the_top_ip_address_from_the_list__top_ip_address = json.loads(phantom.get_run_data(key='get_the_top_ip_address_from_the_list:top_ip_address'))
    # collect data for 'add_dest_ip_artifact' call
    formatted_data_1 = phantom.get_format_data(name='prepare_dest_ip_artifact')

    parameters = []
    
    # build parameters list for 'add_dest_ip_artifact' call
    parameters.append({
    ...
    phantom.act(action="add artifact", parameters=parameters, assets=['phantom asset'], callback=fortimanager_block_dest_ip, name="add_dest_ip_artifact")

    return

"""
Prepare dest IP artifact
"""
@phantom.playbook_block()
def prepare_dest_ip_artifact(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None, custom_function=None, **kwargs):
    phantom.debug('prepare_dest_ip_artifact() called')
    
    template = """{{\"destinationAddress\": \"{0}\", \"destinationAddress_malicious\": \"True\"}}"""
    ...
    phantom.format(container=container, template=template, parameters=parameters, name="prepare_dest_ip_artifact", separator=", ")

    add_dest_ip_artifact(container=container)
...
def decision_10(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None, custom_function=None, **kwargs):
    ...
    matched = phantom.decision(
        ...
        action_results=results,
        conditions=[
            ["artifact:*.cef.destinationAddress_ContainResult", "==", True],
        ],
        scope="all")
