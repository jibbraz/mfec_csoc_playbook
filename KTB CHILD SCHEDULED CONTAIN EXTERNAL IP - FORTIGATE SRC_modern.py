...
def get_the_top_ip_address_from_the_list(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None, custom_function=None, **kwargs):
    ...
    import ipaddress
    success, message, iplist = phantom.get_list(list_name='Fortigate - Src IP to contain')
    for ip in iplist:
        try:
...
def remove_the_top_ip_address_from_the_list(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None, custom_function=None, **kwargs):
    ...
    phantom.delete_from_list(list_name="Fortigate - Src IP to contain", value=get_the_top_ip_address_from_the_list__top_ip_address, column=None, remove_all=False, remove_row=False)
    success, message, iplist = phantom.get_list(list_name='Fortigate - Src IP to contain')
    for ip in iplist:
        try:
            ipaddress.ip_address(ip[0])
...
def add_src_ip_artifact(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None, custom_function=None, **kwargs):
    ...
    phantom.act(action="add artifact", parameters=parameters, assets=['phantom asset'], callback=fortigate_block_src_ip, name="add_src_ip_artifact")
...
"""
Fortigate Block Src IP
"""
@phantom.playbook_block()
def fortigate_block_src_ip(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None, custom_function=None, **kwargs):
    phantom.debug('fortigate_block_src_ip() called')
    
    get_the_top_ip_address_from_the_list__top_ip_address = json.loads(phantom.get_run_data(key='get_the_top_ip_address_from_the_list:top_ip_address'))
    
    # call playbook "local/PLAYBOOK-CONTAIN-INDICATOR-CONSOLIDATED-FORTIGATE-BLOCKSRCIP", returns the playbook_run_id
    playbook_run_id = phantom.playbook(playbook="local/PLAYBOOK-CONTAIN-INDICATOR-CONSOLIDATED-FORTIGATE-BLOCKSRCIP", container=container, name=f"fortigate_block_src_ip_{get_the_top_ip_address_from_the_list__top_ip_address}", callback=sleep)
